...
FROM (
    SELECT tr.*, 
           ...
           t.icon AS therapist_icon
    FROM (
        SELECT therapist_id, available
        FROM (
            SELECT therapist_id,
             
              -- ✅ PROTECTED RANGE: only create tsrange if valid
              CASE
        ...
        WHERE available IS NOT NULL                     -- ⬅ remove invalid slots
...
LEFT JOIN therapists_services tss 
       ON ajde.therapist_id = tss.therapist_id
LEFT JOIN services s 
       ON tss.service_id = s.id AND s.id != 99999
...
const getBookingDetails = (userId, timestamp) => {
  ...
  WHERE user_id = $1 AND time_range @> tsrange($2, $2, '[]')) b
